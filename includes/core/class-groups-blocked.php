...
class Groups_Blocked extends \um\core\Member_Directory {
	...
	function calculate_pagination( $directory_data, $result ) {
		...
		$this->profiles_per_page = apply_filters("um_groups_profiles_per_page__members_blocked",  $this->profiles_per_page );
	...
	function prepare_users_response( $user_ids, $directory_data, $group_id ) {
		...
		foreach ( $user_ids as $i => $user_id ) {
			...
			$users[ $i ]['date'] = sprintf( __( 'Joined %s ago', 'um-groups' ), human_time_diff( strtotime( $user_data['date_joined_gmt'] ), strtotime( gmdate( 'Y-m-d H:i:s' ) ) ) );

			$joined = ! empty( $user_data['date_joined_gmt'] );
